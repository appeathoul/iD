...
import osmtogeojson from 'osm-and-geojson';
...
function osmToGeoJSON(data, tile, mergeCache) {
    var _features = osmtogeojson.osm2geojson(data, true);
    var features = [];
    for (var i = 0; i < _features.features.length; i++) {
...
function loadTile(source, tile) {
    ...
    var extent = tile.extent.toParam();
    ...
    fetch(url, { signal: controller.signal })
        ...
        .then(function (data) {
            ...
            source.loaded[tile.id] = osmToGeoJSON(data, tile, source.canMerge[z]);
